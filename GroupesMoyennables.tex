...
sur les groupes localement compacts, et en particulier le théorème d'existence et unicité des mesures de Haar 
...
Haar, nous notons donc $\mathscr{L}^\infty(\Gamma)$ et $\mathrm{L}^\infty(\Gamma)$.
...
\begin{proof}
    ...
    supposons que $g$ est de la forme $\ev_\varphi$ pour $\varphi\in\mathrm{L}^\infty(X, \mu)$. On a alors, pour $u\in\mathrm{L}^1(X, \mu)_{1, +}$ :
\begin{align*}
    \ket{u, \Re\varphi - a} 
        ...
        &\ge 0
\end{align*}
\footnotetext{Cette égalité provient de ce que $u$ est à valeurs réelles.}Tout élément positif de $\mathrm{L}^1(X, \mu)$ étant nul ou multiple positif d'un élément de $\mathrm{L}^1(X, \mu)_{1, +}$
\footnote{Plus précisément, si $u\in\mathrm{L}^1(X,\mu)$ est positif et non nul, on a $\integral{}{}{u}{\mu}=\norm{u}_{\mathrm{L}^1}\ne 0$. On peut alors poser 
$\widetilde{u}:=\frac1{\integral{}{}{u}{\mu}}\cdot u\in\mathrm{L}^1(X, \mu)_{1, +}$, de sorte que
...
% C'est faux :
%\footnote{Cette notation est justifiée par le fait que ces morphismes
...
Le dernier théorème qui nous intéresse permet de caractériser la moyennabilité en terme de contenance faible.
...
\begin{proof}[Démonstration du théorème \ref{amenable_weak_contain}]
    ...
    On peut alors appliquer l'inégalité de Cauchy-Schwartz, ce qui donne pour $\gamma\in F$ :
    \begin{equation*}
...
%Enfin, nous supposons connus les définitions de tribu et de mesure,
...
%\begin{proof}
%    Si $G$ est $\sigma$-compact, la preuve est identique à la preuve usuelle pour $G = \R^n$. Voyons donc comment s'y ramener.
%
%    Tout d'abord, si $p, q, r$ sont tous finis, on commence par montrer le résultat pour les fonctions continues à support compact.
%    On applique pour cela le lemme \ref{exists_sigma_compact_subgroup}, appliqué à l'union des supports des fonctions en jeu,
%    pour se ramener à un groupe $\sigma$-compact.
%\end{proof}
...
\bibliography{GroupesMoyennables}
