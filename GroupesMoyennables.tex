...
\begin{proof}[Démonstration du lemme \ref{exists_sigma_compact_subgroup}]
    ...
    $\Gamma'$ est borélien, donc la restriction de $\mu$ à $\Gamma'$ est bien définie, et localement compact 
...
\begin{lemma}\label{Reiter_filter_of_cond}
    ...
    \begin{equation}\label{weak_Reiter_cond}\tag{WR}
        \forall\varepsilon>0, \forall \gamma\in\Gamma, \exists \varphi\in\mathscr{L}^1(\Gamma, \mu)_{1,+}, 
...
\begin{proof}
    ...
    Le théorème \ref{dense_in_means} fournit un filtre $\mathscr{F}$ sur $\mathrm{L}^1(\Gamma, \mu)_{1, +}$
