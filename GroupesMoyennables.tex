...
\begin{align*}
    \norm{f^*}_{\mathrm{L}^1} = \text{\TODO{calculer}} = \norm{f}_{\mathrm{L}^1}
\end{align*}
...
\begin{align*}
    (f_1^*\ast f_2^*)(x) 
        &= \integral{}{}{\overline{f_1(g\inv) f_2(x\inv g)}}{\mu(g)} \\
        &= \overline{\integral{}{}{f_1(g\inv x\inv) f_2(g)}{\mu(g)}} \\
        &= \overline{(f_2\ast f_2)(x\inv)} \\
        &= (f_2\ast f_1)^*(x)
\end{align*}

On veut maintenant associer à toute représentation \emph{unitaire} $\pi : \Gamma\to\mathcal{GL}(H)$ du \emph{groupe} $\Gamma$ une
...
Comme $V_\pi$ est un espace de Hilbert, l'isométrie $\ev:V_\pi\to V_\pi^{**}$ est surjectiv, et on pose alors $\pi(f)(x) := \ev\inv(\underline{\pi}(f)(x))$. 
...
\begin{theorem}\label{strong_measurable_crit}
    ...
    $E$ un $\K$-espace de Banach, pour $\K\in\set{\R, \C}$, et $f:X\to E$ une fonction continue. 
    \begin{enumerate}[(i)]
        \item\label{strong_measurable_of_compact} Si $X$ est compact, alors $f$ est fortement mesurable.
        \item\label{strong_measurable_of_L1} Pour toute fonction $\varphi : X\to\K$ intégrable, la fonction $\varphi\cdot f$ est fortement mesurable.
    \end{enumerate}
\end{theorem}

Notons d'ailleurs que, dans le cas $(ii)$, l'intégrabilité est immédiate une fois la mesurabilité forte établie, 
puisque l'inégalité de Hölder pour les fonctions scalaires $\varphi$ et $\norm{\cdot}\comp f$ assure que 
$\norm{\cdot}\comp(\varphi f)$ est intégrable avec $\norm{\varphi f}_{\mathrm{L}^1} = \integral{}{}{\abs{\varphi(x)}\norm{f(x)}}{\mu(x)} \le \norm{\varphi}_{\mathrm{L}^1}\norm{f}_\infty$.

\begin{proof}
    Montrons d'abord que $(\ref{strong_measurable_of_compact})$ entraîne $(\ref{strong_measurable_of_L1})$.
    \TODO{finir}
\end{proof}
