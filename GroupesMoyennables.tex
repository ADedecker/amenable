...
Fixons $\Gamma$ un groupe topologique séparé et localement compact. La notion qui va nous intéresser dans ce mémoire est la suivante.

\begin{definition}
    Le groupe topologique $\Gamma$ est \emph{moyennable} s'il admet une 
    quasi-mesure de probabilité borélienne invariante par translations et absolument continue par rapport à une 
    mesure de Haar sur $\Gamma$.
\end{definition}

Comme toutes les mesures de Haar (à droite et à gauche) sont absolument continues les unes par rappost aux autres,
l'aboslue continuité par rapport à \emph{une} mesure de Haar est équivalente à l'absolue continuité par rapport à \emph{toute}
mesure de Haar. 
...
\begin{proposition}\label{bilateral_of_left}
    Supposons qu'il existe une quasi-mesure de probabilité borélienne $m$ sur $\Gamma$, invariante par translations \emph{à gauche}.
    Alors $\Gamma$ est un groupe moyennable.
\end{proposition} 

\begin{proof}
    Il s'agit donc de construire une \emph{autre} quasi-mesure $n$ sur $\Gamma$ qui soit cette-fois invariante des deux côtés.
    Posons d'abord, pour $A\in\Bor(G)$, $\fundef{f_A}{\Gamma&\to& \Gamma}{g&\mapsto& m(Ag\inv)}$. Chaque $f_A$ est bornée par $1 = m(\Gamma)$, 
    donc intégrable pour la quasi-mesure de probabilité $\invop_*m$. Posons alors $n(A) := \integral{}{}{f_A}{(\invop_*m)}$. \\
...
En effet, la mesure de Haar normalisée d'un tel groupe est une (quasi-)mesure de probabilité invariante par tranlation. Les phénomènes
...
de mesure de probabilité invariante par translations d'après le théorème \ref{theorem_Haar}. C'est donc l'affaibilissement de la 
...
\begin{proof}
    ...
    On montre de même que $b\inv S(b) = S(b\inv)\compl$. Supposons alors qu'il existe une quasi-mesure de probabilité $m$ sur 
...
Sans grande surprise, nous allons maintenant vouloir appliquer différents outils de l'analyse fonctionelle à l'étude des moyennes invariantes.
...
$B(\Gamma)$ engendré par les fonctions de la forme $x\mapsto f(g\inv x) - f(x)$ pour $f\in B(\Gamma)$. 
Il est alors clair qu'une forme linéaire sur $B(\Gamma)$ est une moyenne invariante à gauche \ssi sa restriction à $L_0(\Gamma)$ est nulle. 
...
Un filtre $\mathscr{F}$ sur $\mathcal{K}_+(\Gamma)$ est dit \emph{simplement de F\o{}lner} si le filtre $\mathfrak{f}_*\mathscr{F}$ image directe 
...
Si la convergence est uniforme sur les compacts, on parle de filtre \emph{uniformément de F\o{}lner}.
Une suite $K:\N\to\mathcal{K}_+(\Gamma)$ est \emph{simplement (resp. uniformément) de F\o{}lner} si le filtre $K_*\nhds_{+\infty}^\N$, image directe par $K$ du filtre des
voisinages de l'infini dans $\N$, est simplement (resp. uniformément) de F\o{}lner. Comme $\mathfrak{f}_*K_*\nhds_{+\infty}^\N = (\mathfrak{f}\comp K)_*\nhds_{+\infty}^\N$,
une suite $K:\N\to\mathcal{K}_+(\Gamma)$ est donc simplement (resp. uniformément) de F\o{}lner \ssi la suite de fonctions
...
\begin{lemma}
    Le groupe séparé et localement compact $\Gamma$ admet un filtre simplement de F\o{}lner \ssi il satisfait la 
    \emph{condition de F\o{}lner simple} :
    \begin{equation}\label{simple_Folner_cond}
        \forall\varepsilon>0, \forall \gamma\in\Gamma, \exists K\in\mathcal{K}_+(\Gamma), 
        \frac{\mu(\gamma K\triangle K)}{\mu(K)}<\varepsilon
    \end{equation}
    Il admet de un filtre uniformément de F\o{}lner \ssi il satisfait la \emph{condition de F\o{}lner uniforme} : 
    \begin{equation}\label{uniform_Folner_cond}
...
Comme anoncé, on peut alors généraliser le théorème \ref{Z_amenable} à tout groupe muni d'un filtre (simplement) de F\o{}lner,
en adaptant simplement la deuxième preuve de ce théorème.

\begin{theorem}\label{amenable_of_Folner}
    Si $\Gamma$ admet un filtre simplement de F\o{}lner, alors $\Gamma$ est moyennable.
\end{theorem}
...
\begin{proof}
    ...
    Soit finalement $\mathscr{F}$ un filtre simplement de F\o{}lner pour $\Gamma$. L'estimation précédente assure alors que
