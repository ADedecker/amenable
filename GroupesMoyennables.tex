...
Nous utiliserons à de nombreuses reprises le lemme \ref{EVT_emb_TFAE} pour caractériser le spectre approché d'un opérateur $T$,
...
\begin{proof}
    ...
    Pour montrer l'implication \framebox{$(\ref{amenable_weak_contain/weak_contain})\implies(\ref{amenable_weak_contain/norm_eq_two})$}, commençons par remarquer 
    que l'inégalité $\norm{\lambda_\Gamma(\gamma)+\lambda_\Gamma(\gamma\inv)}\le2$ est automatiquement vérifée, il s'agit donc de montrer l'intégalité 
    inverse. 
    \TODO{commentaire cas discret}
    %Notons que si $\Gamma$ est discret, on peut directement appliquer $(\ref{amenable_weak_contain/weak_contain})$ à la fonction $f := \delta_\gamma + \delta_{\gamma\inv}$,
    %d'intégrale 
    Fixons pour cela une fonction 
    $f_0\in\mathscr{L}^1(\Gamma)_{1, +}$\footnote{On a déjà vu qu'une telle fonction existe toujours, par exemple en posant $f_0:=\chi_U=\frac{1}{\mu(U)}\indic_U$ pour 
    un voisinage compact $U$ de $1$.}. Soit alors $\gamma\in\Gamma$ quelconque, et considérons les opérateurs $T := \lambda_\Gamma(\gamma) + \lambda_\Gamma(\gamma\inv) \in\mathcal{L}(\mathrm{L}^2(\Gamma))$,
    ...
    \end{equation*}
    Notons que, pour tout $f\in\mathscr{L}^1(\Gamma)$ positive, la fonction $S(f)$ est encore positive, et vérifie $\integral{}{}{S(f)}{\mu} = 2\integral{}{}{f}{\mu}$.
    ...
    et sa norme est égal à son rayon spectral, de sorte que $\set{-2, 2}\cap\Sp(T)\ne\varnothing$. On va maintenant 
