...
\begin{proof}
    Supposons d'abord qu'on ait $1 =_\mu^{loc} \lambda(\gamma)(f) - f$ pour certains $\gamma\in\Gamma$ et 
    ...
    Pour le cas général, on suppose cette fois $1 =_\mu^{loc} \sum_{i\in I} (\lambda(\gamma_i)(f_i) - f_i)$ pour $I$ fini, $\gamma : I \to\Gamma$ et
...
Au vu de l'assertion $(\ref{amenable_weak_contain/one_mem_spectrum})$, il est assez naturel d'interpréter le point
$(\ref{amenable_weak_contain/weak_almost_invariant})$ comme l'existence de \og{}vecteurs propres approchés\fg{} pour la valeur propre $1$.
Cela nous ammène à introduire la notion standard de \emph{spectre approché} d'un opérateur.
...
\begin{proof} 
    ...
    Il est clair que $(\ref{EVT_emb_TFAE/lower_bound_norm}) \iff(\ref{EVT_emb_TFAE/lower_bound_norm'})$, il reste donc à montrer $(\ref{EVT_emb_TFAE/emb}) \iff(\ref{EVT_emb_TFAE/lower_bound_norm})$. Notons que $(\ref{EVT_emb_TFAE/lower_bound_norm})$ implique
...
\begin{proof}
    ...
    un $\gamma\in\Gamma$ quelconque, et commençons par remarquer 
    que l'inégalité $\norm{\lambda_\Gamma(\gamma)+\lambda_\Gamma(\gamma\inv)}\le2$ est automatiquement vérifée, il s'agit donc de montrer l'intégalité 
    ...
    à la fonction $f := \delta_\gamma + \delta_{\gamma\inv}\in\mathscr{L}^1(\Gamma)_{+}$ pour obtenir :
    \begin{equation*}
        2 = \abs{\integral{}{}{f}{\mu}} = \abs{\indic_\Gamma(f)} \le \norm{\lambda_\Gamma(f)} = \norm{\lambda_\Gamma(\gamma)+\lambda_\Gamma(\gamma\inv)}\le2
    \end{equation*}

    Revenons maintenant au cas général. On pourrait être tenté de se ramener au raisonnement précédent 
    en approchant la \emph{mesure} $\delta_\gamma + \delta_{\gamma\inv}$ par des fonctions intégrables,
    ...
    même si $f\in\mathscr{L}^1(\Gamma, \mu)$ a sa masse concentrée autour de $\gamma$ et $\gamma\inv$,
    rien ne dit que $\lambda_\Gamma(f)$ sera proche de $\lambda_\Gamma(\gamma) + \lambda_\Gamma(\gamma\inv)$. 
    ...
    La solution que nous proposons est d'étudier ce qui se passe lorsqu'on itère notre opérateur $\lambda_\Gamma(\gamma) + \lambda_\Gamma(\gamma\inv)$.
    Fixons pour cela une fonction $f_0\in\mathscr{L}^1(\Gamma)_{1, +}$\footnote{On a déjà vu qu'une telle fonction existe toujours, par exemple en posant $f_0:=\chi_U=\frac{1}{\mu(U)}\indic_U$ pour 
    un voisinage compact $U$ de $1$.} qui représente une distribution de masse initiale, et considérons les opérateurs $T := \lambda_\Gamma(\gamma) + \lambda_\Gamma(\gamma\inv) \in\mathcal{L}(\mathrm{L}^2(\Gamma))$
    et $S := \lambda(\gamma) + \lambda(\gamma\inv) \in \mathcal{L}(\mathrm{L}^1(\Gamma))$. Par \TODO{ref ou argument à base de convolution}, on a :
    \begin{equation*}
        \forall f\in\mathrm{L}^1(\Gamma), \lambda_\Gamma(S(f)) = T \comp \lambda_\Gamma(f)
    \end{equation*}
    Notons que, pour toute $f\in\mathscr{L}^1(\Gamma)$ positive, la fonction $S(f)$ est encore positive, et vérifie $\integral{}{}{S(f)}{\mu} = 2\integral{}{}{f}{\mu}$.
    Par récurrence, on a donc $\forall n\in\N, S^n(f_0)\geq0 \land \integral{}{}{S^n(f_0)}{\mu} = 2^n$. Or, $(\ref{amenable_weak_contain/weak_contain})$
    assure que l'on a, toujours pour $n\in\N$ quelconque :
    \begin{equation*}
        2^n = \integral{}{}{S^n(f_0)}{\mu} = \norm{\indic_\Gamma(S^n(f_0))} \leq \norm{\lambda_\Gamma(S^n(f_0))} = \norm{T^n \comp \lambda_\Gamma(f_0)} \le \norm{T}^n \norm{\lambda_\Gamma(f_0)}
    \end{equation*}
    La suite géométrique positive $n\mapsto\left(\frac{2}{\norm{T}}\right)^n$ est donc bornée supérieurement par $\norm{\lambda_\Gamma(f_0)}$, ce qui assure que $\frac{2}{\norm{T}} \leq 1$,
    et donc $2\leq\norm{T}$ comme attendu.
...
%
%\begin{lemma}
%    $\pi$ admet presque des vecteurs invariants \ssi $\forall\varepsilon>0, \forall f\in\mathscr{L}^1_{1, +}(\Gamma)\cap C_c(\Gamma), \exists\xi\in V_\pi, 
...
%
%\begin{proof}
%    En supposant d'abord que $\pi$ admet presque des vecteurs invariants, soient $\varepsilon>0$ et $f\in\mathscr{L}^1_{1, +}(\Gamma)\cap C_c(\Gamma)$,
