...
\TODO{ATTENTION pour avoir ${\mathrm{L}^1}^*\simeq\mathrm{L}^\infty$ il faut $\sigma$-finitude !!!!!}
...
\begin{proof}[Démonstration du lemme \ref{positive_of_positive_form}]
    \TODO{Cette preuve est fausse !}
    Supposons que l'ensemble $\mathbf{S} := \set{x\in X\tq\varphi(x)<0}$ est de mesure non nulle. Il existe alors 
    $\mathbf{T}\subseteq\mathbf{S}$ de mesure finie non nulle (\TODO{c'est faux}), de sorte que $\indic_\mathbf{T}$ soit positive et intégrable. L'intégrale
